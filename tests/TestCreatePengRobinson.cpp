...
TEST_CASE("Can create and evaluate an EoS", "[PengRobinson]"){
    ...
    CHECK(A1.size() == 1); CHECK(Catch::Approx(PhaseBehavior::Math::roundUp(A1[0],4))==0.1480);
    CHECK(Catch::Approx(PhaseBehavior::Math::roundUp(A2[0],4))==0.6059);
    CHECK(Catch::Approx(PhaseBehavior::Math::roundUp(A2[1],4))==0.1267);
    CHECK(A3.size() == 1); CHECK(Catch::Approx(PhaseBehavior::Math::roundUp(A3[0],4))==0.7368);
    CHECK(A4.size() == 1); CHECK(Catch::Approx(PhaseBehavior::Math::roundUp(A4[0],4))==0.6609);
    CHECK(A5.size() == 1); CHECK(Catch::Approx(PhaseBehavior::Math::roundUp(A5[0],4))==0.7420);
    CHECK(A6.size() == 1); CHECK(Catch::Approx(PhaseBehavior::Math::roundUp(A6[0],4))==0.8141);
}
